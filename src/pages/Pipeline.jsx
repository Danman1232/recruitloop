...
import { getPipeline, updateSubmission, getJobs } from '../services/api';
import { getStageColor, formatStageLabel } from '../utils/stageHelpers';

// Mirror CompanySubmissions STAGES exactly
const STAGES = [
...
// helpers to mirror CompanySubmissions behavior
...
export default function Pipeline() {
  ...
  const job = jobsById.get(jobIdKey);

  const jobLocation = (jobObj) => {
    if (!jobObj) return '';
    const composed = [jobObj.city ?? jobObj.jobCity, jobObj.state ?? jobObj.jobState]
      .filter(Boolean)
      .join(', ');
    return jobObj.location ?? jobObj.jobLocation ?? composed ?? '';
  };
  ...
  useEffect(() => {
    (async () => {
      setLoading(true);
      try {
        // Only pull submissions for THIS job, and fetch jobs to derive location
        const [pipelineSubs, allJobs] = await Promise.all([
          getPipeline(jobIdKey),
          getJobs(),
        ]);
        setSubs(pipelineSubs || []);
        setJobs(allJobs || []);
      } catch {
        console.error('Failed to load pipeline/jobs');
      } finally {
        setLoading(false);
      }
    })();
  ...
  // rows: only subs for this job (getPipeline already scoped) filtered by stage/status
  const rows = subs.filter(s => (s.stage || s.status) === stage);

  // Dynamic column header identical to CompanySubmissions
  const dateHeader =
    stage === 'offer_sent' ||
  ...
  async function patch(id, updates) {
    // optimistic mirror of CompanySubmissions
    setSubs(list => list.map(x => x.id === id ? { ...x, ...updates } : x));
    try {
      await updateSubmission(id, updates);
    } catch {
      console.error('Failed to update submission');
      // (optional) revert here if strict consistency is required
    }
  }

  return (
    <main className="flex-1 flex h-screen bg-gray-100 overflow-hidden">
      {/* Sidebar (mirrors CompanySubmissions) */}
      ...
      {/* Main (mirrors CompanySubmissions) */}
      <div className="flex-1 flex flex-col overflow-hidden">
        <div className="px-6 pt-6 pb-4">
          <h2 className="text-3xl font-bold">Submissions</h2>
          {/* Optional subtitle showing the job context; remove if you want pixel-perfect identity */}
          {job ? (
            <p className="text-gray-500 mt-1">
              {job.title} • {jobLocation(job) || '—'}
            </p>
          ) : null}
        </div>
          ...
          {loading ? (
            ...
            <table className="min-w-full bg-white rounded shadow overflow-hidden">
              ...
              <tbody className="divide-y">
                {rows.map(s => {
                  // derive location exactly like the CompanySubmissions change
                  const derivedLoc =
                    s.location ||
                    s.jobLocation ||
                    jobLocation(job) ||
